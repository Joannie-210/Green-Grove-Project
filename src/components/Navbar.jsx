...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => setScrolled(window.scrollY > window.innerHeight - 10);
  ...
  return (
    <header className="w-full fixed top-0 left-0 z-50 flex justify-center items-center h-[100px]">
      <nav
        className={clsx(
          'w-full max-w-screen-xl px-4 py-2 flex items-center justify-between rounded-full transition-all duration-300 ease-in-out',
          scrolled ? 'bg-black/90 shadow-lg text-white' : 'bg-white/20 backdrop-blur-sm shadow-md text-green-500'
        )}
      >
        {/* Logo */}
        <NavLink to="/">
          <img src={Logo} alt="Home" className="w-16 h-16 object-cover rounded-full" />
        </NavLink>
        ...
        >
          {navLinks.map(({ to, label }) => (
            <li key={to} className="group relative text-center mb-3 hover:text-green-500">
              <NavLink
                to={to}
                className={({ isActive }) =>
                  clsx(
          ...
          <li className="relative group w-full md:w-auto">
            <div
              ...
              onClick={() => window.innerWidth < 768 && setProductsOpen(!productsOpen)}
            >
            ...
            >
              <li><NavLink to="/tools" className="block px-4 py-2 hover:text-green-100">Gardening Tools</NavLink></li>
              <li><NavLink to="/essential" className="block px-4 py-2 hover:text-green-100">Gardening Essentials</NavLink></li>
              <li><NavLink to="/accessory" className="block px-4 py-2 hover:text-green-100">Accessories</NavLink></li>
              <li><NavLink to="/article" className="block px-4 py-2 hover:text-green-100">Related Articles</NavLink></li>
            </ul>
          </li>
        </ul>
        ...
        <div className="hidden md:flex flex-col items-end text-sm text-white ml-4">
          <span className="text-[11px] flex gap-1 items-center">
            <UserRound size={14} /> {visitCount} {visitCount === 1 ? 'visit' : 'visits'}
          </span>
        </div>
      </nav>
    </header>
